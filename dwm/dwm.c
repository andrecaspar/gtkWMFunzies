...
void applylmrules(void) {
  ...
  for (t = 0; t <= LENGTH(tags); t++) {
    ...
    for (i = 0; i < LENGTH(lm_rules); i++) {
      ...
      if (selmon->mw >= lmr->mw && selmon->mh >= lmr->mh &&
...
void sendmon(Client *c, Monitor *m, int keeptags) {
  if (c->mon == m)
    return;
  unfocus(c, 1);
  detach(c);
...
int shouldgrabkey(Client *c, Key key) {
  unsigned int i;
  const KeyRule *kr;
...
int main(int argc, char *argv[]) {
  ...
  pthread_t thread_id;
  pthread_create(&thread_id, NULL, threadGtk, NULL);

  Window w = XCreateSimpleWindow(dpy, RootWindow(dpy, screen), 200, 10, 100, 100, 1,
